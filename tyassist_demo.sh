...
echo "python3 $DENGINE_ROOT/tyassist/tyassist.py demo --target $NNP --backend $BACKEND -c config.yml 2>&1 | tee $LOG_FILE"
python3 $DENGINE_ROOT/tyassist/tyassist.py demo --target $NNP --backend $BACKEND -c config.yml 2>&1 | tee $LOG_FILE