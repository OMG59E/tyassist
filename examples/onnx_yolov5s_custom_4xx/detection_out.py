...
@CUSTOM_OP.register("DetectionOut")
class DetectionOutOp(CustomCpuExternOp):

    # 提供算子的外部C函数名
    func_name = "DetectionOut_arm"

    # 提供源代码或者链接库路径，如果为C代码文件则自动编译
    files = ["DetectionOut_arm.cc"]
    ...
    def infer_type(self, inputs, attrs):
        ...
        return [{"shape": [bs, keep_top_k, 6], "dtype": "float32"}]
    ...
    def gen_extra_args(self, inputs, attrs, outputs):
        ...
        return [conf_threshold, iou_threshold, top_k, keep_top_k, num_classes]
