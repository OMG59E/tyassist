...
echo "python3 $DENGINE_ROOT/tyassist/tyassist.py test --target $NNP --backend $BACKEND $EXTRA -c config.yml 2>&1 | tee $LOG_FILE"
python3 $DENGINE_ROOT/tyassist/tyassist.py test --target $NNP --backend $BACKEND $EXTRA -c config.yml 2>&1 | tee $LOG_FILE